import torch
import torch.nn as nn
import torchvision.transforms.functional as TF

class Conv2dRelu(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(Conv2dRelu, self).__init__()
        self.conv = nn.Conv2d(
            in_channels,
            out_channels,
            kernel_size=3,
            padding=1
        )

        self.relu = nn.ReLU(inplace=True)

    def forward(self, x):
        x = self.conv(x)
        x = self.relu(x)
        return x
    
class DoubleConv2dRelu(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(DoubleConv2dRelu, self).__init__()
        self.conv1 = Conv2dRelu(in_channels, out_channels)
        self.conv2 = Conv2dRelu(out_channels, out_channels)

    def forward(self, x):
        x = self.conv1(x)
        x = self.conv2(x)
        return x

class TripleConv2dRelu(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(TripleConv2dRelu, self).__init__()
        self.conv1 = Conv2dRelu(in_channels, out_channels)
        self.conv2 = Conv2dRelu(out_channels, out_channels)
        self.conv3 = Conv2dRelu(out_channels, out_channels)

    def forward(self, x):
        x = self.conv1(x)
        x = self.conv2(x)
        x = self.conv3(x)

        return x

class VGG16(nn.Module):
    def __init__(self, in_channels, out_channels):
        super(VGG16, self).__init__()


        self.ups = nn.ModuleList()
        self.downs = nn.ModuleList()
        self.pool = nn.MaxPool2d(kernel_size=2, stride=2)

        self.downs.append(DoubleConv2dRelu(in_channels, 64))
        self.downs.append(DoubleConv2dRelu(64, 128))
        self.downs.append(TripleConv2dRelu(128, 256))
        self.downs.append(TripleConv2dRelu(256, 512))

        self.bottleneck = TripleConv2dRelu(512, 512 * 2)

        self.ups.append(
            nn.ConvTranspose2d(
                512 * 2,
                512,
                kernel_size=2,
                stride=2
            )
        )
        self.ups.append(TripleConv2dRelu(512 * 2, 512))
        self.ups.append(
            nn.ConvTranspose2d(
                256 * 2,
                256,
                kernel_size=2,
                stride=2
            )
        )
        self.ups.append(TripleConv2dRelu(256 * 2, 256))
        self.ups.append(
            nn.ConvTranspose2d(
                128 * 2,
                128,
                kernel_size=2,
                stride=2
            )
        )
        self.ups.append(DoubleConv2dRelu(128 * 2, 128))
        self.ups.append(
            nn.ConvTranspose2d(
                64 * 2,
                64,
                kernel_size=2,
                stride=2
            )
        )
        self.ups.append(DoubleConv2dRelu(64 * 2, 64))

        self.final_conv = nn.Conv2d(64, out_channels, kernel_size=1)

    def forward(self, x):
        skip_connections = []

        for down in self.downs:
            x = down(x)
            skip_connections.append(x)
            x = self.pool(x)

        x = self.bottleneck(x)

        skip_connections = skip_connections[::-1]

        for idx in range(0, len(self.ups), 2):
            x = self.ups[idx](x)
            skip = skip_connections[idx // 2]

            if x.shape != skip.shape:
                x = TF.resize(x, size=skip.shape[2:])

            concat_skip = torch.cat((skip, x), dim=1)
            x = self.ups[idx + 1](concat_skip)

        return self.final_conv(x)